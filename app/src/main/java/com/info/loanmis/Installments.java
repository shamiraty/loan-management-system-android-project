...
public class Installments extends Fragment {

    //get preference to select specific login customer
    String SHARED_PREF_XML = "user_details_xml";
    private List<ModelInstallments> list;
    private AdapterInstallments adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        list = new ArrayList<>();
        adapter = new AdapterInstallments(getContext(), list);
        ...
        StringRequest request = new StringRequest(Request.Method.POST, "https://projectforce.000webhostapp.com/loan/installments.php", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                dialog.dismiss();

                //make sure database exceptions are handled
                try {
                    ...
                    for (int loop = 0; loop < array.length(); loop++) {
                        ...
                        list.add(new ModelInstallments(object.getString("id"),
                                object.getString("InstalledAmount"),
                                object.getString("National_ID"),
                                object.getString("created_at"),
                                object.getString("EmployeeUsername")));
